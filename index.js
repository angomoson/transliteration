...
class Mapper {
  constructor() {
    ...
    for (const [key, value] of Object.entries(MEITEI_MAYEK_NUMBERS)) {
      ...
      this.phonemes.set(
        value,
        new Phoneme(value, false, "", key, false, "", true)
      );
    }
  ...
  getOutput() {
    switch (this.outputMode) {
      case OutputMode.VOWEL:
        return this.phoneme.asVowel;
      case OutputMode.CONSONANT:
        return this.phoneme.asConsonant;
      case OutputMode.LONSUM:
        return this.phoneme.asLonsum;
    }
        ...
        if (curr.isVowel) {
          ...
          if (curr.phoneme === "ng") {
            ...
            if (MAPPER.isApunMayekPhonemesCombo(prev.phoneme, curr)) {
              output.push(
                new PhonemeOutput(APUN_MAYEK_AS_PHONEME, OutputMode.CONSONANT)
              );
            }

            // use as lonsum greedily if possible. But no consecutive lonsums
            const next = new PhonemeOutput(
              curr,
              curr.canBeLonsum
                ? prev.outputMode != OutputMode.LONSUM
                  ? OutputMode.LONSUM
...
app.post("/users", (req, res) => {
  ...
  const transliteratedText = mmTransliterator.transliterate(data);
  res.send(`${transliteratedText}`);
});
